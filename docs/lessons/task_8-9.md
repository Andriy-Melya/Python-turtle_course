...
---
## 4️⃣ Прямокутник по точках із сторонами `x` та `y` пікселів 

Створи програму, у якій черепашка намалює прямокутник зі сторонами `x` та `y` пікселів, лівий нижній кут має координати (0, 0):
...
### Підказка:
`x` та `y` - це змінні, які визначають ширину і висоту прямокутника  
...
Створи програму, у якій черепашка намалює квадрат зі стороною `a` пікселів, верхній правий кут має координати `(x0, y0)`:
...
`x0` та `y0` — координати верхнього правого кута.
...
Ліва точка основи має координати `(x0, y0)`.
...
`h` — висота трикутника.  
`x0`, `y0` — координати лівої точки основи.
...
Нижня вершина ромба має координати `(x0, y0)`.
...
`x0`, `y0` — координати нижньої вершини ромба
...
Початкова точка — **правий нижній кут** трикутника з координатами `(x0, y0)`.
...
`x0`, `y0` — координати правого нижнього кута.
...
Початкова точка — **лівий нижній кут** паралелограма з координатами `(x0, y0)`.
...
`x0`, `y0` — координати лівого нижнього кута.
...
Центр ромба має координати `(x0, y0)`.
